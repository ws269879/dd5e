...
export class TraitsComponent implements OnInit {
  ...
  constructor(private _apiService: ApiService,
    ...
    this.rowClickSubject.subscribe((alignment) => {
      this._router.navigate(['app', 'traits', alignment.index])
    })
  }

  async ngOnInit() {

    let traits = await this._apiService.getTraits()

    if (!traits) {
      traits = {count: 0, results: []}
    }

    // Assign the data to the data source for the table to render
    this.dataSource = new MatTableDataSource(traits.results);
  }
}
